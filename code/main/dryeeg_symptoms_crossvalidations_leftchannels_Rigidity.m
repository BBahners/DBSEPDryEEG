...
rmappath=dryeeg_result_path(repoRoot, 'symptoms_crossvalidations_Rigidity'); % results path
...
tab=readtable(dryeeg_data_path(repoRoot, 'metadata/UPDRS_dryEEG_ImprovementsOmitNanUp_Rigidity.xlsx'));
...
ax=gcf;
%ax.Colormap=cmap;
cmp=ea_colorgradient(256,[1 1 1],[1 1 1],[0.898 0.4314 0.2588]);
ax.Colormap=cmp;
...
cb = colorbar();
        ...
        ax=gca;
        ax.TickDir="out";
        saveas(h5,[rmappath,'topo_',timwinlab{iwin},'_orange','.png']);
    end
