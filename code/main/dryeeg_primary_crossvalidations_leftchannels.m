...
rmappath=dryeeg_result_path(repoRoot, 'primary_crossvalidations_leftchannels'); % results path
if ~exist([rmappath])
    mkdir([rmappath]);
end
% Machine-specific addpath removed; see config/local_paths.m
...
folds=[58,10, 7, 5, 2];
foldcell={'LOO-CV', '10-fold-CV','7-fold-CV','5-fold-CV','Split-half-CV'};
for ifolds=1:length(folds)
    ...
    %    [I,Ihat, ~, val_struct] = crossval('kfold',c,[],0,silent);
    ...
    for i=1:c.NumTestSets
        ...
        Rmap=bb_Rmap_ep(patConnectivityFiles(idx_training),Regressor(idx_training),[rmappath,'Rmap_train',num2str(i),'.mat'],corrtype);
    ...
    for i=1:c.NumTestSets
        ...
        testsubs=find(idx_test)';
        for pt=testsubs
            mdl{pt}=fitlm(RegressorHat(idx_training),Regressor(idx_training)*100);
            Pred(pt)=predict(mdl{pt},RegressorHat(pt));
            MAEpts(pt) = mean(abs(Pred(pt)-Regressor(pt)));
            RMSpts(pt)=mdl{pt}.RMSE;
        end
    end
    ...
    [h,R,p,g]=ea_corrplot(Regressor*100,Pred','no',{foldcell{ifolds},'% UPDRS Improvement (Empirical)','% UPDRS Improvement (Estimated)'},group1,[],cmap(210,:),[],'linear');
    RMS=mean(RMSpts);
    MAE=mean(MAEpts);
  ...
  %  saveas(h,[rmappath,'Results_leftchannels_pred_',foldcell{ifolds},'.png']);
   % save([rmappath,'Results_leftchannels_pred_',foldcell{ifolds},'.mat'],'RegressorHat','Regressor','MAE','RMS','R','p');
    ...
    [h,R,p,g]=ea_corrplot(Regressor*100,RegressorHat','no',{foldcell{ifolds},'% UPDRS Improvement','Similarity to R-Matrix'},group1,[],cmap(25,:),[],'linear');%{'LOOCV Crossvalidation','Empirical Regressor','Similarity to R-Map'}
    h.Position(4)=h.Position(3)+h.Position(3)*0.025;
 h.Children(2).FontWeight="bold";
    h.Children(2).TickDir= "out";
    h.Children(2).FontSize=14;
    saveas(h,[rmappath,'Results_leftchannels_',foldcell{ifolds},'.png']);
    save([rmappath,'Results_leftchannels_',foldcell{ifolds},'.mat'],'RegressorHat','Regressor','R','p');
